...
void Testbench::do_blur() {
  int32_t x, y;         // for loop counter
  pixel p, result;
  int32_t to_filter_cnt = 0, to_testbench_cnt = 0; // number of pixel transfer

  o_rst.write(false);
  o_rst.write(true);
  for (y = 0; y < IMAGE_H + 2; ++y) {
    for (x = 0; x < IMAGE_W + 2; ++x) {
      if (y != 0 && y != (IMAGE_H + 1) && x != 0 && x != (IMAGE_W + 1))
        p.set(*(source_bitmap +
      ...
      if (y > 1 && x > 1)
      {
        if(i_p.num_available() == 0) wait(i_p.data_written_event());
        result = i_p.read();
        to_testbench_cnt++;
        *(target_bitmap + bytes_per_pixel * (width * (y - 2) + (x - 2)) + 2) = result.r();
        *(target_bitmap + bytes_per_pixel * (width * (y - 2) + (x - 2)) + 1) = result.g();
        *(target_bitmap + bytes_per_pixel * (width * (y - 2) + (x - 2)) + 0) = result.b();
      }
    }
  }
  cout << "Number of pixel transfer" << endl;
  cout << "- to filter: " << to_filter_cnt << endl;
  cout << "- to testbench: " <<  to_testbench_cnt << endl;
  cout << "- total: " << to_filter_cnt + to_testbench_cnt << endl;
  sc_stop();
}